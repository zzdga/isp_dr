...
options:
  ...
  disks:
    description:
      ...
      - Only the listed disks will be part of the DG,
        meaning if the disk is removed from the list it will also be removed from the DG
    default: None
  redundancy:
    description:
...
def create_diskgroup(cursor, module, msg, oracle_home, name, disks, redundancy, attribute_name, attribute_value):
    ...
    if add_attr:
        attributes = ','.join(
            ['\'' + str(n) + '\'' + '=' + '\'' + str(v) + '\'' for n, v in zip(attribute_name, attribute_value)])
    else:
        attributes = ''
    ...
    if execute_sql(module, msg, cursor, sql):
        if rac:
            ...
            if rc != 0:
                # 'Edge-case', where there is only one instance in the cluster.
                # The diskgroup is already running after create statement so this command errors
                if 'CRS-5702' in stdout:
                    return True
                else:
                    msg[0] = 'Error, couldn\'t mount the dg on all nodes. stdout: %s, stderr: %s, command is %s' % (
...
def remove_diskgroup(cursor, module, msg, oracle_home, name):
    ...
    if rac:
        ...
        (rc, stdout, stderr) = module.run_command(command)
        if rc != 0:
            msg[0] = 'Error, couldn\'t unmount the dg. stdout: %s, stderr: %s, command is %s' % (
                stdout, stderr, command)
            return False
...
def ensure_diskgroup_state(cursor, module, msg, name, state, disks, attribute_name, attribute_value):
    ...
    if sorted(list_current_path) == sorted(list_wanted) and not change_attr:
...
def main():
    ...
    if not cx_oracle_exists:
        msg[0] = "The cx_Oracle module is required. 'pip install cx_Oracle' should do the trick." \
    ...
    try:
        ...
        error, = exc.args
        msg[0] = 'Could not connect to ASM: %s, connect descriptor: %s, username: %s, pass: %s' % (
    ...
    if state == 'present':
        if not check_diskgroup_exists(cursor, module, msg, name):
            if create_diskgroup(cursor, module, msg, oracle_home, name, disks, redundancy, attribute_name,
                                attribute_value):
