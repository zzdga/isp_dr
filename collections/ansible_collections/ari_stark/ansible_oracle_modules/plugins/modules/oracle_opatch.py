...
def apply_patch(module, msg, oracle_home, patch_base, patch_id, patch_version, opatchauto, ocm_response_file, offline,
                stop_processes, rolling, output):
    """
    Applies the patch
    """
        ...
        with open(oratabfile) as oratab:
            ...
            for line in oratab:
                if line.startswith('#') or line.startswith(' '):
                    ...
                    p = subprocess.Popen('ps -elf| grep "[0-9] %s/bin/tnslsnr"' % (line.split(':')[1]), shell=True,
                    ...
                    p = subprocess.Popen('ps -elf| grep "[0-9] ora_pmon_%s"' % (line.split(':')[0]), shell=True,
                                         stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    output, error = p.communicate()
                    p_status = p.wait()
...
def main():
    ...
    if state == 'present':
        if not check_patch_applied(module, msg, oracle_home, patch_id, patch_version, opatchauto):
            if apply_patch(module, msg, oracle_home, patch_base, patch_id, patch_version, opatchauto, ocm_response_file,
